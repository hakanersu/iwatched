                            ...
                            <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                                <div>
                                    <h3 class="text-2xl leading-6 font-medium text-gray-900"
                                        v-text="series.original_title"></h3>
                                ...
                                <dl>
                                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 flex items-center">
                                            ...
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 mr-2 text-xs font-bold leading-none text-red-100 bg-indigo-600 rounded-full ml-3">+{{ directors.length }}</span>
                        ...
                        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                            <div class="flex space-x-3">
                                ...
                                <div v-if="seasonsNumbers.length>8">
                                    <div class="relative inline-block text-left">
                                        ...
                                        <div v-if="showDropdown" style="width: 180px;"
                                             class="origin-top-right bg-white space-y-3 absolute right-0 mt-2 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                                             role="menu" aria-orientation="vertical" aria-labelledby="menu-button"
                                             tabindex="-1">
                                            <div class="max-h-48 overflow-y-auto p-2">
                                                <template v-for="season in seasonsNumbers" :key="season">
                                                    <season-tab :class="{'mb-2': Number(season) !== totalSeasons}"
                                                                :label="`Season ${season}`"
                                                                :active="Number(season) === selectedSeason"
                                                                @click="selectedSeason = Number(season)"
                                                                v-if="Number(season) >= 8"/>
                            ...
                            <div
                                ...
                                <button
                                    ...
                                    :class="[isAllWatched ? 'bg-white shadow text-green-700': '']"
                                    @click="markAllWatched()"
                                >
                                        ...
                                        <tr>
                                            <td class="px-4 py-5 whitespace-nowrap"
                                                ...
                                                <div class="tg-list-item">
                                                    <input class="tgl tgl-flat" :id="`check-${episode.tconst}`"
                                                           @change="watchEpisode"
                                                           type="checkbox" :value="episode.tconst" v-model="selected">
                                                    <label :for="`check-${episode.tconst}`" class="tgl-btn"></label>
                                                </div>
                                            </td>
                                            <td class=" px-4 text-gray-800 w-full max-w-0"
                                                :class="[index % 2 === 0 ? 'bg-gray-50' : 'bg-white']">
                                                {{ episode.original_title }}
                                            </td>
                                            <td class=" px-4 text-gray-600  whitespace-nowrap"
                                                :class="[index % 2 === 0 ? 'bg-gray-50' : 'bg-white']">
                                                <span
                                                    class="text-xs opacity-50">{{ `S${episode.season_number}E${episode.episode_number}` }}</span>
                                            </td>
                                        </tr>
                                    </template>
                                </table>
...
export default defineComponent({
    ...
    computed: {
        ...
        isAllWatched() {
            return this.watchedSeasons.every((item) => item === true)
        }
    },
    data: () => ({
        selected: [],
    ...
    mounted() {
        Object.keys(this.seasons).forEach((item) => {
            this.isSeasonWatched(item - 1)
        })

        this.selected = this.episodes.filter(item => item.watched.length > 0).map(item => item.tconst)
    },
    methods: {
        watchEpisode(e) {
        ...
        toggle(season, request = true) {
            ...
            episodes.forEach((episode) => {
                if (this.watchedSeasons[season]) {
                    this.selected = this.selected.filter(item => item !== episode)
                }else {
                    if (this.selected.indexOf(episode) === -1) {
        ...
        isSeasonWatched(season) {
            const seasons = this.seasons[season + 1]

            if (this.selected.length <= 0 || !seasons) {
                this.watchedSeasons[season] = false
                return false
            }
            const episodes = seasons.map(item => item.tconst)

            const isWatched = this.selected !== [] && this.isInArray(episodes, this.selected)

            this.watchedSeasons[season] = isWatched

            return isWatched
        },
        isInArray(mainArray, subArray) {
            return mainArray.every(elem => subArray.includes(elem))
        },
        parse(value) {
