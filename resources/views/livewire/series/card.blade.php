<div class="bg-white min-h-full flex flex-col shadow-md rounded-md">
    ...
    <div class="flex flex-col flex-1 justify-between h-full">
        <div class="p-3">
            ...
            <h5 class="text-gray-500 text-sm">{{ $series->genres }}</h5>
        </div>
        <div class="flex text-gray-800 p-3 justify-between items-center">
            <h3>{{ $series->rating->average_rating ?? '-' }}</h3>
            <div class="flex items-center">
                <h3>{{ $series->start_year }}</h3>
                <a href="https://www.imdb.com/title/{{ $series->tconst }}" target="_blank" class="ml-2">
